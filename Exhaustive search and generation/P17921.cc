...
void reines (int n, int filas, vector<bool>& columnas, vector<bool>& diagonales1, vector<bool>& diagonales2, Matriz & mat ) {  
    if (filas == n) escribir_matriz(mat);
    for(int j = 0; j < n; ++j) {
        ...
        if ( not columnas[j] and not diagonales1[n -filas + j] and not diagonales2[filas + j] ) {
            ...
            reines(n,filas +1,columnas,diagonales1,diagonales2,mat);
            columnas[j] = false;
...
int main () {
    ...
    int contador = 0;
    reines(n,0,columnas,diagonales1,diagonales2,mat);
